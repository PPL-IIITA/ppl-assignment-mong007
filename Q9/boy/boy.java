...
public class boy implements Comparable<boy> {
    ...
    */ 
	public char view_status() {
         ...
         * @param g list of object of girl class.
         ...
         */
        public int mk_pair(girl[] g, int giv) {
            int ct = giv;
          for(girl j : g) {
              if(ct>0) {
			if(j.status=='s' && this.status=='s' && this.is_eligible(j.budget, j.attract ) && j.is_eligible(this.budget)) {
                            j.status = 'c';
				this.status = 'c';
				j.bf = this.name;
				this.gf = j.name;
