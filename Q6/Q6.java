...
Random r = new Random();
int t = r.nextInt(50);
breakup(t,b,g,c,ct2,cs);
        }
        /**
 * This function performs breakup according to randomly generated value of t.
 * @param t no. of days.
 ...
 * @param cs Arraylist of couples.
        */
 public static void breakup(int t, boy b[], girl g[], couple c[], int ct2,List<couple> cs) {
try {
 ...
 while (i < ct2) {
                    if(cs.get(i).happ < t){
                        cs.get(i).b.status = 's';
                            cs.get(i).g.status = 's';
                            cs.get(i).b.ex_id = cs.get(i).g.bf_id;
                            cs.get(i).g.ex_id = cs.get(i).b.gf_id ;
                            cs.get(i).b.gf_id = -1;
                            cs.get(i).g.bf_id = -1;
...
file1.write("New couples formed"+"\n");
    for(i=0;i<100;i++) {
		for(int j=99;j>-1;j--) {
			if(g[i].ex_id!= j && b[j].ex_id !=i && g[i].status=='s' && b[j].status=='s' && g[i].is_eligible(b[j].budget) && b[j].is_eligible(g[i].budget, g[i].attract)) {
                            g[i].status = 'c';
				b[j].status = 'c';
				g[i].bf = b[j].name;
				b[j].gf = g[i].name;
